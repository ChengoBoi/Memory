...
class MemoryGame:
    ...
    def __init__(self, fönster, ord_fil="memo.txt", highscore_fil="highscore.txt"): #Parameter. Tkinter-Widget
    ...
    def bygg_gränssnitt(self):
        for widget in self.fönster.winfo_children():
            widget.destroy()
        ...
        for rad in range(rader):
            ...
            for kol in range(kolumner):
                knapp = tk.Button(
                    ...
                    command=lambda r=rad, k=kol: self.vald_ruta(r, k)
                )
...
if __name__ == "__main__":
    huvud_fönster = tk.Tk()
    huvud_fönster.title("Memory-spel")
